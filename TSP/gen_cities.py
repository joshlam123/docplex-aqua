...
class GenCities:
    ...
    def __init__(self, no_cities=100):
        self.start_city = 0.0
        self.cities = dict()
        self.n = no_cities

        self.i1 = np.arange(0, 0 + round((self.n**0.5),2), 0.01)
        self.i2 = np.arange(0, 0 + round((self.n**0.5),2), 0.01)
    ...
    def precomp_distances(self):
        ...
        for (k, v) in self.cities.items():
            self.table_distances[k] = {i:self.distance(self.cities[i][0], self.cities[k][0]) for i,j in self.cities.items()}
